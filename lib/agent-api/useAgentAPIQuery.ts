...
export function useAgentAPIQuery(params: AgentApiQueryParams) {
    ...
    React.useEffect(() => {
        setCurrentUsername(getUsernameFromCookie());
    }, []);
            ...
            if (type === "text" && text !== undefined) {
                ...
                if (toolResultsResponse?.tool_results) {
                    ...
                    if (statement) {
                        ...
                        const statement2 = `SET TENANT = '${currentUsername}'; ${statement}`
                        ...
                        const sqlExecUserMessage = getSQLExecUserMessage(latestUserMessageId, tableData.statementHandles[1])
                        const { headers, body } = buildStandardRequestParams({
                            authToken,
                        ...
                        for await (const event of data2AnalyticsStreamEvents) {
                            ...
                            if (event.event == "execution_trace") continue;
                            const {
                                delta: {
